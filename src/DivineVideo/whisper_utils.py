...
def generate_audio_srt(audio_file):
    ...
    print(f"Executing whisper command: whisper {audio_file} --language {language} --model {model} -f {output_format} -o {data_dir}/audio/output/")

    try:
        cmd = ["whisper", audio_file, "--language", language, "--model", model, "-f", output_format, "-o", f"{data_dir}/audio/output"]
